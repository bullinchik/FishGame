    ...
    public class TTPPrivacySettings
    {
        ...
        private class EditorImpl : IPrivacySettings
        {
            ...
            public EditorImpl()
            {
                ...
                string path = TTPUtils.CombinePaths(new List<string>(){Application.streamingAssetsPath, "ttp", "configurations","privacySettings.json"});
                ...
                {
                    #if UNITY_ANDROID
                    path = TTPUtils.CombinePaths(new List<string>(){Application.streamingAssetsPath, "ttp", "templateconfig", "gp","privacySettings.json"});
                    #elif UNITY_IOS
                    path = TTPUtils.CombinePaths(new List<string>(){Application.streamingAssetsPath, "ttp", "templateconfig", "ios","privacySettings.json"});
                    #endif
                }
                else if (Application.platform == RuntimePlatform.Android)
                {
                    androidPath = TTPUtils.CombinePaths(new List<string>(){"ttp", "configurations", "privacySettings.json"});
                }
                else if (Application.platform == RuntimePlatform.IPhonePlayer)
                {
                    path = TTPUtils.CombinePaths(new List<string>(){Application.streamingAssetsPath, "ttp", "configurations","privacySettings.json"});
                }
                Debug.Log("TTPPrivacySettings::config path=" + path);
                string privacySettingsJson;
                if (androidPath != null)
                {
                    privacySettingsJson = TTPUtils.ReadStreamingAssetsFile(androidPath);
                }
                else
                {
                    privacySettingsJson = System.IO.File.ReadAllText(path);
                }

...
namespace Tabtale.TTPlugins
{
    ...
    public class TTPAnalytics
    {
        ...
        [Preserve]
        public static void LogEvent(string eventName, IDictionary<string, object> eventParams, bool timed, bool ttpInternal = false)
        {
            LogEvent(AnalyticsTargets.ANALYTICS_TARGET_FIREBASE, eventName, eventParams, timed, ttpInternal);
        }
        /// <summary> Logs an analytics event to one or more analytics provider. </summary>
        ...
        /// <param name="eventParams">dictionary of key-values that describe the event</param>
        /// <param name="timed">if this event should be timed, meaning it will only be sent when EndLogEvent() is sent, with an additional parameter that denotes how much time it took.</param>
        [Obsolete("Use method LogEvent(string eventName, IDictionary<string, object> eventParams, bool timed, bool ttpInternal = false) instead")]
        [Preserve]
        ...
        private class EditorImpl : IAnalytics
        {
            ...
            {
#if TTP_CORE
                var config = TTPTestAB.Config;
                if (config != null && config.ContainsKey(key))
                {
                    var value = config[key] as string;
                    Debug.Log("TTPAnalytics::EditorImpl::GetRemoteStringSync: key=" + key + "value=" + value);
                    return value;
                }
                else
                {
                    Debug.Log(
                        "TTPAnalytics::EditorImpl::GetRemoteStringSync: there is no a key value pair for the provided key: key= " +
                        key);
                    return "";
                }
#endif

...
#region IStoreListener implementation
                        ...
                        if (appleReceipt != null)
                        {
                            ...
                            foreach (TTPAppleInAppPurchaseReceipt inAppPurchaseReceipt in appleReceipt.inAppPurchaseReceipts)
                            {
                                Debug.Log("ProcessPurchaseDebug : productID=" + inAppPurchaseReceipt.productID +
                                          " originalTransactionIdentifier=" + inAppPurchaseReceipt.originalTransactionIdentifier +
        ...
        private void LogEventByReflection(long targets, string eventName, Dictionary<string,object> parameters)
        {
            ...
            {
                var method = analyticsClsType.GetMethod("LogEvent", new Type[] { typeof(long), typeof(string), typeof(IDictionary<string,object>), typeof(bool), typeof(bool) });
                if (method != null)
                {
        ...
        {
            LogEventByReflection((ANALYTICS_TARGET_DELTA_DNA | ANALYTICS_TARGET_FIREBASE), TTPEvents.SUBSCRIPTION_STARTED, postData);
        }
